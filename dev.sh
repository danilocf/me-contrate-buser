...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=me-contrate-buser.example.com
...
workon me-contrate-buser  # Change this to your project's name
...
function dkbuild {
    CD=$(pwd)
    cd $PROJ_BASE
    docker build -t me-contrate-buser .
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development me-contrate-buser npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop me-contrate-buser
    docker rm me-contrate-buser
    docker run --name me-contrate-buser -d --env-file /home/ubuntu/me-contrate-buser.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/me-contrate-buser:/dkdata \
        me-contrate-buser start_web.sh
}

function deploy_prod {
  ssh ubuntu@$HOST_PROD "
    cd ~/me-contrate-buser
    git reset --hard
    git pull
    source dev.sh
    dkbuild
...
function dk {
    docker exec -it me-contrate-buser $@
}
...
echo_green "Welcome to me-contrate-buser's dev env"
